...
- 상품 상세 페이지
  - [ ] 참고 URL 을 참고하여 상품 상세 페이지 구현
  - [ ] API 통신의 최적화, 선언적 구조, react query 등을 활용하여 기능 구현
  - [ ] 아래 참고 API를 모두 사용하여 구현하기
  ...
  - [ ] 상품 관심 등록 버튼 클릭 시 낙관적 업데이트를 통해 상품 관심 등록 수를 변경할 수 있도록 구현
        (실제 API 반영은 없기 때문에, 새로고침 하면 사라지는 것이 정상)

- [ ] ErrorBoundary와 Suspense를 사용하여 코드 구조 리팩터링 하기
