...
<상품 상세 - API 연동 심화, 테스트>
    ...
    [v] 3단계  테스트 코드 작성하기
        ...
        (+) 로그인 페이지 테스트 시나리오
            ...
            - 시나리오 3: 이메일과 비밀번호에 에러 메시지가 있을 때 화면에 에러 메시지 표시
                - 목적: 이메일 또는 비밀번호에 에러 메시지가 있을 경우, 해당 메시지가 화면에 노출되는지 확인
                - 조건:
                    이메일 필드에 에러 메시지 노출 ("이메일 형식이 올바르지 않습니다")
                    비밀번호 필드에 에러 메시지 노출 ("비밀번호는 8자 이상이어야 합니다")
                - 예상 결과:
                    각각의 에러 메시지가 화면에 표시된다.
                    로그인 버튼은 비활성화 상태다.

            - 시나리오 4: 로그인 버튼 클릭 동작 확인 (내부 함수 호출 여부는 테스트하지 않음)
                - 목적: 로그인 버튼이 활성화 상태에서 클릭 가능한지 확인
                - 조건: 유효한 이메일과 비밀번호가 입력되어 로그인 버튼이 활성화된 상태
                - 예상 결과:
                    로그인 버튼을 클릭할 수 있다.
                    내부 goToLogin 함수 호출 여부는 테스트하지 않는다.
