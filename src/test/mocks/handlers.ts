...
const filterProductsByParams = (
  ...
  if (target === 'FEMALE' && rank === 'MANY_WISH')
    return productsMock.slice(0, 4);
  if (target === 'MALE' && rank === 'MANY_RECEIVE')
    return productsMock.slice(2, 6);
  if (target === 'TEEN' && rank === 'MANY_WISH_RECEIVE')
    return productsMock.slice(4, 8);
  return productsMock;
};
...
export const handlers = [
  http.get('/api/products/ranking', ({ request }) => {
    ...
    const target = searchParams.get('targetType') ?? 'ALL';
    const rank = searchParams.get('rankType') ?? 'MANY_WISH';

    const filtered = filterProductsByParams(target, rank);
