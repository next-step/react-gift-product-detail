...
export default function ProductDetailContent() {
  ...
  const {
    ...
    queryFn: () =>
      axios.get<{ data: Info }>(`/api/products/${productId}`)
  ...
  const {
    ...
    queryFn: () =>
      axios.get<{ data: Detail }>(`/api/products/${productId}/detail`)
           .then(res => res.data.data),
  });
  ...
  const {
    ...
    queryFn: () =>
      axios.get<{ data: WishData }>(`/api/products/${productId}/wish`)
           .then(res => res.data.data),
  });

  // 클릭 시 로컬 캐시만 업데이트 (낙관적)
  const handleToggleWish = () => {
    queryClient.setQueryData<WishData>(
      ['product', productId, 'wish'],
      old => {
  ...
  return (
    <Container>
      ...
      <Content>
        ...
        {tab === 'rev' && reviews.map(r => (
          <ReviewItem key={r.id}><strong>{r.authorName}</strong>: {r.content}</ReviewItem>
        ))}
        {tab === 'info' && (
          <InfoList>
...
// --- Styled components ---
const Loading = styled.div`padding:2rem; text-align:center;`;
const Container = styled.div`display:flex; flex-direction:column;`;
const Header = styled.div`text-align:center; padding:1rem;`;
const ProductImage = styled.img`width:100%; height:auto; object-fit:cover;`;
const Title = styled.h1`margin:1rem 0 .5rem; font-size:1.5rem;`;
const Price = styled.p`font-size:1.25rem; font-weight:bold; color:#333;`;
const TabWrapper = styled.nav`display:flex; border-bottom:1px solid #ddd;`;
...
const DescriptionContainer = styled.div`line-height:1.6; img{max-width:100%;height:auto;}`;
const ReviewItem = styled.div`margin-bottom:1rem;`;
const InfoList = styled.ul`list-style:none; padding:0;`;
const InfoItem = styled.li`margin-bottom:1rem;`;
const InfoName = styled.div`font-weight:bold;`;
const InfoValue = styled.div`color:#555;`;
const EmptyInfo = styled.div`color:#999;`;
const Footer = styled.div`display:flex; position:sticky; bottom:0; background:#fff; border-top:1px solid #ddd;`;
const WishButton = styled.button<{ isWished: boolean }>`
  flex:1; padding:1rem; background:#fff; border:none; cursor:pointer;
`;
const OrderButton = styled.button`flex:2; padding:1rem; background:#ffd400; border:none; font-weight:bold; cursor:pointer;`;
